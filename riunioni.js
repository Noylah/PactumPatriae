...
function renderEditableODG(odgRaw) {
    ...
    header.innerHTML = `
        <span class="tiny-label" style="margin:0;">Punti e Decisioni</span>
        <button type="button" class="save-btn" onclick="aggiungiPuntoODG()" style="width:auto; padding:5px 15px; font-size:0.7rem;">+ AGGIUNGI PUNTO</button>
    `;
...
function aggiungiNomeExtra(targetContainerId) {
    ...
    label.innerHTML = `
        <input type="checkbox" name="${targetContainerId === 'listaAppello' ? 'presenti' : 'editPresenti'}" value="${nome}" checked>
        <span class="pill-content" style="border-color: rgba(255,255,255,0.2);">
            <span class="pill-text">${nome} (Ex)</span>
        </span>
    `;
...
async function fetchConsiglieriPerAppello() {
    ...
    container.innerHTML = `
        <div style="width:100%; display:flex; gap:10px; margin-bottom:15px;">
            <input type="text" id="inputExtraNuovo" class="minimal-input" placeholder="Aggiungi Ex Consigliere..." style="flex:1;">
            <button type="button" class="save-btn" onclick="aggiungiNomeExtra('listaAppello')" style="width:45px; height:45px; padding:0; display:flex; align-items:center; justify-content:center; font-size:1.5rem;">+</button>
        </div>
    `;
    data.forEach(c => {
        const label = document.createElement('label');
...
async function salvaRiunione() {
    ...
    const { error } = await _supabase.from('riunioni').insert([{ data: dataVal, presidiata_da: chiVal, odg: odgVal, presenti: selezionati }]);
...
async function fetchRiunioni() {
    ...
    riunioni.forEach(r => {
        ...
        trMain.innerHTML = `
            <td><span style="color:#d4af37; margin-right:8px;">✦</span>${r.data}</td>
            <td>${r.presidiata_da}</td>
            <td colspan="2" style="text-align: center; font-size: 0.75rem; opacity: 0.5; font-style: italic;">Dettagli</td>
            <td style="text-align: right;">
                <button class="btn-action-dash edit" onclick="event.stopPropagation(); modificaRiunione(${r.id})">MODIFICA</button>
                <button class="btn-action-dash delete" onclick="event.stopPropagation(); eliminaRiunione(${r.id})" style="background:rgba(255,77,77,0.1); color:#ff4d4d; border:1px solid rgba(255,77,77,0.2); margin-left:5px;">ELIMINA</button>
            </td>
        `;
        ...
        trDetails.innerHTML = `
            <td colspan="5">
                <div style="padding:20px; border-left:2px solid #d4af37; margin:10px 0; display:grid; grid-template-columns:1fr 1fr; gap:40px;">
                    <div><span style="color:#d4af37; font-size:0.7rem; text-transform:uppercase; letter-spacing:1px; display:block; margin-bottom:15px;">Ordine del Giorno</span>${odgHTML}</div>
